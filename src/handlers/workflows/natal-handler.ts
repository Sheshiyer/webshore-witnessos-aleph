/**
 * Natal Chart Analysis Workflow Handler for WitnessOS API
 *
 * Handles natal chart analysis workflows that combine multiple consciousness
 * engines to provide comprehensive birth chart interpretations and personality insights.
 */
...
export class NatalHandler extends BaseWorkflowHandler {
  ...
  /**
   * Get the engines used for natal chart analysis
   */
  getWorkflowEngines(): EngineName[] {
    return ['numerology', 'human_design', 'vimshottari', 'gene_keys'];
  }

  /**
   * Handle natal workflow requests
   */
  async handle(request: Request, requestId?: string): Promise<Response> {
    ...
    if (request.method !== 'POST') {
      return this.createErrorResponse(405, 'METHOD_NOT_ALLOWED', 'Only POST method allowed for natal workflows', reqId);
    }
    ...
    try {
      ...
      if (!authResult.success || !authResult.user) {
        return this.createErrorResponse(401, 'UNAUTHORIZED', 'Authentication required for natal workflows', reqId);
      }
      ...
      if (!requestData.userProfile.birthDate || !requestData.userProfile.birthTime || !requestData.userProfile.birthLocation) {
        return this.createErrorResponse(400, 'MISSING_BIRTH_DATA', 'Complete birth data (date, time, location) is required for natal analysis', reqId);
      }
      ...
      console.error(`[${reqId}] Natal workflow error:`, error);
      return this.createErrorResponse(500, 'WORKFLOW_FAILED', 'Natal workflow execution failed', reqId);
    }
  }
}
