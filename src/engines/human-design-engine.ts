...
export class HumanDesignEngine extends BaseEngine<HumanDesignInput, HumanDesignOutput> {
  ...
  private buildChartFromAstronomicalData(astronomicalData: any): any {
    try {
      ...
      if (!personalityGates || !designGates) {
        throw new Error('Missing personality or design gates in astronomical data');
      }

      console.log('🌟 Personality gates:', JSON.stringify(personalityGates, null, 2));
      console.log('🌙 Design gates:', JSON.stringify(designGates, null, 2));

      // Determine type based on gate activations
      const type = this.determineTypeFromGates(personalityGates, designGates);
      ...
      const centers = this.calculateCentersFromGates(personalityGates, designGates);
      console.log('🏛️ Centers:', centers);

      // Calculate authority based on defined centers
      const authority = this.calculateAuthorityFromCenters(centers);
      console.log('⚖️ Authority:', authority);

      // Build incarnation cross
      const incarnationCross = {
        name: `Cross of ${personalityGates.SUN?.gate || 1}/${personalityGates.EARTH?.gate || 2}`,
        gates: [
          personalityGates.SUN?.gate || 1,
          personalityGates.EARTH?.gate || 2,
          designGates.SUN?.gate || 1,
          designGates.EARTH?.gate || 2
        ],
        description: `Incarnation Cross based on gates ${personalityGates.SUN?.gate || 1}, ${personalityGates.EARTH?.gate || 2}, ${designGates.SUN?.gate || 1}, ${designGates.EARTH?.gate || 2}`
      };
      console.log('✨ Incarnation Cross:', incarnationCross);
      ...
      const chart = {
        ...
        personalityGates,
        designGates,
        definition: 'Real Astronomical'
      };

      console.log('🎉 Complete chart built successfully:', JSON.stringify(chart, null, 2));
      return chart;

    } catch (error) {
      console.error('❌ Error building chart from astronomical data:', error);
      console.error('📊 Astronomical data received:', JSON.stringify(astronomicalData, null, 2));
      throw error;
    }
  }
