...
export default {
  async fetch(request: Request, env: EngineProxyEnv): Promise<Response> {
    ...
    try {
      ...
      // Get engine metadata
      if (path.startsWith('/engines/') && path.endsWith('/metadata') && method === 'GET') {
        ...
        try {
          // Forward metadata request to Railway backend
          const response = await fetch(`${RAILWAY_BASE_URL}/engines/${engine}/metadata`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              'User-Agent': 'WitnessOS-EngineProxy/1.0'
            }
          });
          ...
          console.error(`Metadata error for ${engine}:`, error);
          return new Response(JSON.stringify({
            success: false,
            error: `Failed to get metadata for engine: ${engine}`
          }), {
            status: 500,
            headers: {
              'Content-Type': 'application/json',
