-- Phase 1 Infrastructure Scaling: Performance Indexes Migration (FIXED)
...
-- Fixed: Removed non-deterministic datetime() function from partial index

-- Drop the problematic index if it exists (cleanup from previous attempt)
...
-- User sessions performance indexes (deterministic only)
CREATE INDEX IF NOT EXISTS idx_user_sessions_user_expires ON user_sessions(user_id, expires_at DESC);
-- Note: For active session queries, we'll rely on the expires_at index and filter in application code
